...
def any_mode_with_fun():
    # 3. MODE = ANY (Force Function Call) - With allowed_function_names
    print("--- MODE = ANY (With allowed_function_names) ---")
    ...
    flist = ["get_indian_festival_date"]
    uconfig = protos.FunctionCallingConfig(mode=umode, allowed_function_names = flist)
    
    tool_config_any = protos.ToolConfig(function_calling_config = uconfig)

    MODEL = "gemini-2.0-flash-001"
    model = GenerativeModel(MODEL)

    response = model.generate_content(prompt, tools=[indian_context_tool], tool_config=tool_config_any)
