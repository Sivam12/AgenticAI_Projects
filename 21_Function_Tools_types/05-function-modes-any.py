...
def any_mode_no_fun():
    # 2. MODE = ANY (Force Function Call) - Without allowed_function_names
    print("--- MODE = ANY (No allowed_function_names) ---")
    indian_context_tool  = declare_fun_tool()

    prompt = """
    You are an assistant designed to answer questions using the given tools 
    when appropriate.

    If none of the provided tools are suitable or relevant, 
    and you do not have sufficient information in your own knowledge,
    respond with: "I don’t know" or "None of the tools are relevant to this query."

    DO NOT make assumptions or fabricate an answer.

    User query:
    Tell me something I can use to help understand Indian culture.
    """

    prompt = """
    You are an AI agent with access to certain function tools.

    RULES:
    1. Only call a tool if it directly applies to the user's query.
    2. If no tool applies, reply strictly with:
    "No matching tool available for this request."
    3. Never generate or guess information outside of tool outputs.
    ...
    umode = protos.FunctionCallingConfig.Mode.ANY
