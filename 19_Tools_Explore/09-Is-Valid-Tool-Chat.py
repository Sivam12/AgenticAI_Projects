...
def main():
    ...
    # Initialize Gemini model with tools and create a chat session
    model = GenerativeModel(MODEL, tools=[retail_tool])
    chat = model.start_chat()               # Persistent chat context across turns

    prompt = "Do you have the book titled 'Let Us C'?"

    # Send message using default tools attached to the chat
    response = chat.send_message(prompt)
    print(f"response :\n{response.candidates[0].content.parts[0]}")

    # Send message while overriding/explicitly providing tools for this request
    response = chat.send_message(prompt, tools=[retail_tool])
    print(f"Response:\n{response.candidates[0].content.parts[0]}")
