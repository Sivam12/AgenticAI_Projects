...
def main():
    ...
    chat = init_chat(MODEL)                        # Initialize chat with tools
    
    prompt = "Do you have the book titled 'Let Us C'?"
    response = chat.send_message(prompt)           # First query in the conversation
    print(f"==================={prompt}====================")
    print(f"response :\n{response.candidates[0].content.parts[0]}")

    prompt = "Share me store in Kadapa, AP that I can visit to try one out?"
    response = chat.send_message(prompt)           # Second query (chat history preserved)
    print(f"==================={prompt}====================")
    print(f"response :\n{response.candidates[0].content.parts[0]}")

    prompt = "What is the nearest store in Kadapa, AP?"
    response = chat.send_message(prompt)           # Third query, benefits from context
    print(f"==================={prompt}====================")
    print(f"response :\n{response.candidates[0].content.parts[0]}")
